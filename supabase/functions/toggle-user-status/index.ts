...
serve(async (req) => {
  ...
  try {
    ...
    if (roleData.role !== 'admin') {
      return new Response(
        JSON.stringify({ error: 'Apenas administradores podem ativar/desativar usuários' }),
        { status: 403, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
      );
    }
    ...
    if (!targetUserProfile || targetUserProfile.tenant_id !== roleData.tenant_id) {
      return new Response(
    ...
    // Check if target user is admin - cannot deactivate admins
    ...
    if (targetRole?.role === 'admin') {
      return new Response(
        JSON.stringify({ error: 'Não é possível desativar administradores' }),
        { status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
      );
    }

    console.log(`Toggling user status: ${userId} to ${active}`);

    // Update the profile active status
    ...
    console.log(`User ${userId} status changed to ${active ? 'active' : 'inactive'}`);
